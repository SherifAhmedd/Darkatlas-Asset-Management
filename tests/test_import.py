...
@pytest.mark.asyncio
async def test_bulk_import_creates_relationships(
...
async def test_import_is_idempotent(client: AsyncClient, auth_headers: dict):
    ...
    resp2 = await client.post(
        "/api/v1/assets/import", json=payload, headers=auth_headers
    )
    ...
    initial = [
        {
            ...
            "source": "scan",
            "tags": [],
            "metadata": {"score": 1, "env": "prod"},
        }
    ]
    await client.post(
        "/api/v1/assets/import", json={"assets": initial}, headers=auth_headers
    )
    ...
    updated = [
        {
            ...
            "source": "scan",
            "tags": [],
            "metadata": {"score": 99, "new_key": "hello"},
        }
    ]
    await client.post(
        "/api/v1/assets/import", json={"assets": updated}, headers=auth_headers
    )

    resp = await client.get(
        "/api/v1/assets", params={"value_contains": "merge.com"}, headers=auth_headers
    )
...
async def test_tag_merge_deduplication(client: AsyncClient, auth_headers: dict):
    ...
    await client.post(
        "/api/v1/assets/import", json={"assets": first}, headers=auth_headers
    )
    ...
    await client.post(
        "/api/v1/assets/import", json={"assets": second}, headers=auth_headers
    )

    resp = await client.get(
        "/api/v1/assets", params={"value_contains": "tags.com"}, headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_stale_asset_reactivated_on_reimport(
    client: AsyncClient, auth_headers: dict
):
    ...
    await client.post(
        "/api/v1/assets/import", json={"assets": reimport}, headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_malformed_record_does_not_crash_batch(
    client: AsyncClient, auth_headers: dict
):
...
@pytest.mark.asyncio
async def test_broken_relationship_reference_is_reported(
    client: AsyncClient, auth_headers: dict
):
...
async def test_domain_normalization_lowercase(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.get(
        "/api/v1/assets",
        params={"value_contains": "upper.example.com"},
        headers=auth_headers,
    )
...
@pytest.mark.asyncio
async def test_service_normalization_lowercase_protocol(
    client: AsyncClient, auth_headers: dict
):
    ...
    await client.post("/api/v1/assets/import", json=payload, headers=auth_headers)
    resp = await client.get(
        "/api/v1/assets", params={"asset_type": "service"}, headers=auth_headers
    )
    assert resp.json()["items"][0]["value"] == "443/tcp"


@pytest.mark.asyncio
async def test_normalization_prevents_duplicates(
    client: AsyncClient, auth_headers: dict
):
